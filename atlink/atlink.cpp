...
unsigned char dosStubData[88] = {
    0x4D, 0x5A, 0x60, 0x00, 0x01, 0x00, 0x00, 0x00, 0x04, 0x00, 0x10, 0x00, 0xFF, 0xFF, 0x00, 0x00,
    0xFE, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x58, 0x00, 0x00, 0x00,
    0x4E, 0x4F, 0x20, 0x44, 0x4F, 0x53, 0x21, 0x24, 0x16, 0x1F, 0x33, 0xD2, 0xB4, 0x09, 0xCD, 0x21,
    0xB8, 0x01, 0x4C, 0xCD, 0x21, 0x90, 0x90, 0x90
};
...
void Linker::fixUpReloc(ElfRel & reloc, uint8_t * data, ElfSection * pSection)
{
    ...
    if (file->fileType == FileType::ELF_OBJECT) {
        ...
        if (symbol.shndx == ELF_SECTION_INDEX_UNDEF) {      // 外部全局符号
            ...
            ElfSection * symbolSection = flatSections.sections[flatSectionIndex];
            if (symbol.type == SYMBOL_TYPE_SECTION) {
                value = symbolSection->startImageAddress;
            }
...
void Linker::buildImageFile()
{
    ...
    // COFFFileHeader 头
    coffHeaderFilePos = (uint32_t) ftell(exe);
    ...
    // PE每个section数据必须文件对齐
    uint32_t currFilePointer = (uint32_t) ftell(exe);
    if (currFilePointer % segmentFileAlign != 0) {
        int paddingSize = segmentFileAlign - (currFilePointer % segmentFileAlign);
        for (int i = 0; i < paddingSize; i++)
            fputc(0, exe);
    }
    firstPESectionDataFilePos = (uint32_t)ftell(exe);
    // 
    for (size_t i = 0; i < imageSegments.size(); i++) {
        ImageSegment & segment = imageSegments[i];
        segment.segmentFilePos = get_file_curr_pointer(exe);
